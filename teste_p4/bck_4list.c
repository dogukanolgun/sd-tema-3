...
void solve(struct Graph *graph, struct Queue *q,
           int *directii, int index, int vertices);
...
int main(int argc, char *argv[])
{
    ...
    enQueue(q, 7, 2);
    enQueue(q, 4, 2);
    solve(graph, q, directii, index, counter - 1);
    freeQueue(q);
}
           ...
           int *directii, int index, int vertices)
{
    while (!isEmptyQueue(q))
    {
        ...
        while (temp->weight == next_dir)
        {
            ...
            // solve(graph, directii, ++index, vertices, temp->vertex);
